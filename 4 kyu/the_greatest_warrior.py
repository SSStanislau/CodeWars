...
class Warrior:
    ...
    @property
    def rank(self):
        return RANKS[int(self.experience // 1000)] if self.experience <= 10000 else RANKS[-1]
    
    def battle(self, enemy):
        if enemy < 1 or enemy > 100: 
            return 'Invalid level'
        if self.level >= enemy:
            if self.level - enemy > 1:
                return "Easy fight"
            if self.level - enemy == 1:
                self.experience += 5
                return "A good fight"
            if self.level == enemy:
                self.experience += 10 
                return "A good fight"
        if enemy - self.level > 4 and self.rank != RANKS[enemy // 10]:
            return "You've been defeated"
        else:
            self.experience += 20 * (enemy-self.level)**2
            return "An intense fight"
    
    def training(self, arr):
        if self.level >= arr[2]:
            if self.experience + arr[1] <= 10000:
                self.experience += arr[1]
            else:
